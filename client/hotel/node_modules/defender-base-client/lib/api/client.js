...
class BaseApiClient {
    ...
    async init() {
        if (!this.api) {
            ...
            this.session = await auth_1.authenticate(userPass, poolData);
            this.api = api_1.createAuthenticatedApi(userPass.Username, this.session, this.getApiUrl(), this.httpsAgent);
        }
        return this.api;
    }
    async refresh() {
        ...
        try {
            const userPass = { Username: this.apiKey, Password: this.apiSecret };
            const poolData = { UserPoolId: this.getPoolId(), ClientId: this.getPoolClientId() };
            this.session = await auth_1.refreshSession(userPass, poolData, this.session);
            this.api = api_1.createAuthenticatedApi(userPass.Username, this.session, this.getApiUrl(), this.httpsAgent);
            return this.api;
        }
        catch (e) {
